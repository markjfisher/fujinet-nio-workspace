...
def main() -> int:
    ...
    parser.add_argument("--namespace", default="amiga-e2e-rendezvous")
    parser.add_argument("--ready", default="before-dn2-remount")
    parser.add_argument("--release", default="release-dn2-remount")
    ...
    try:
        rendezvous.wait_for_checkpoint(run_dir, args.namespace, args.ready, args.timeout)
        ...
        rendezvous.release_guest(run_dir, args.namespace, args.release)
        ...
        with transcript.open("a", encoding="utf-8") as out:
            out.write(__import__("json").dumps({"error": repr(error)}) + "\n")
