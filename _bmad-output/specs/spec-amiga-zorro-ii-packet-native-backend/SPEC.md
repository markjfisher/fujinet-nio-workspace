...
- At 2.2: verify the TZT Pico-style RP2040 clone header map/flash configuration, APIO loader seam and instrument capabilities. An eight-channel USB logic analyzer, HANMATEK DOS1102 oscilloscope and simple multimeter are available; analyzer model/input limits/sample rate and scope probe configuration remain to verify. USB diagnostics suffice for initial functional tests; timing/release and later actual-bus evidence retain their measurement gates.
