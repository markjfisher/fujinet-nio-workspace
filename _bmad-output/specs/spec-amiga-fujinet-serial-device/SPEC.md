...
- Pending `CMD_WRITE` is the same one-owner Exec model. `BeginIO` claims `pending_write`, arms TBE, and returns without `ReplyMsg()`. The extra TBE after the last `SERDAT` write means the final byte was accepted by the transmit shift register; that is WRITE complete (not TSRE/wire-idle). A dedicated write software interrupt replies once. `AbortIO`, `CMD_FLUSH`, and final close compete for that same ownership. Do not busy-wait TBE with a CPU iteration limit. `io_Actual` is bytes committed to `SERDAT`.
...
- If a pending WRITE exists when `CMD_FLUSH` is issued, cancel it through the WRITE ownership path with `IOERR_ABORTED` and disable TBE before applying the READ/RX FLUSH behaviour.
- Before removing the RBF/TBE vectors or releasing Paula/`misc.resource` ownership, final close must ensure that no pending IORequest remains retained by the device. Resolve WRITE then READ. Any retained request uses the same one-time cancellation/completion path as `AbortIO`.
