...
Owning repo: `repos/fujinet-nio-driver`. Workspace harness: `integration-tests/amiberry`. Durable hardware procedure: `repos/fujinet-nio-driver/docs/amiga/rs232-cold-warm-hardware-test.md`.
...
| Gate | Command | Covers |
| --- | --- | --- |
| Host | `make -C repos/fujinet-nio-driver/amiga tests` | `test_fujinet_paula_uart` (SERPER 9600 NTSC=371, 38400 PAL=91, 19200 PAL=183; ring/overrun); SET_SERIAL encode/decode; exchange `--serial-device` plan |
...
| Hardware | Cold clock one-shot in `rs232-cold-warm-hardware-test.md` | CAP-5; operator runs this |
...
The serial backend (`fujinet_nio_serial_backend.c`) issues `OpenDevice`, `SDCMD_SETPARAMS` (8N1, `SERF_XDISABLED`, optional `SERF_RAD_BOOGIE`, `io_RBufLen` multiple of 64), `CMD_WRITE` (DoIO, often one SLIP frame), `SDCMD_QUERY`, `CMD_READ` (SendIO + timer abort), `CMD_FLUSH`. Exclusive open is required: the broker is the only opener.
